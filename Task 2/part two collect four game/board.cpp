...
bool CollectFourBoard::update_board (int x, int y, char mark){
    ...
    if (!(x < 0 || x > 6 || y < 0 || y > 7) && isBottom && board[x][y] == 0) {
...
bool CollectFourBoard::is_winner() {
    ...
    for(int i =lastPlayX + min(5-lastPlayX,lastPlayY), j = lastPlayY - min(5-lastPlayX,lastPlayY); i>= min(lastPlayX,3) && j<= min(lastPlayY,3); i--, j++){
        for(int k = 0; k<4 ; k++){
            if( board[i-k][j+k] != lastPlaySymbol ){
                goto st3;
            }
